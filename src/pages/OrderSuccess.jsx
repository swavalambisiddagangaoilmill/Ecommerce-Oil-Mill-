      ...
      <Container className="max-w-5xl">
        <div className="rounded-[2rem] bg-white p-6 shadow-soft sm:p-8 print:shadow-none">
          <div className="text-center">
            ...
            <p className="mt-3 text-ink/60">Your order link is ready to share once backend order URLs are connected.</p>
          </div>
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Info label="Order ID" value={order.id} />
            <Info label="Date & Time" value={order.date} />
            <Info label="Payment Status" value={order.paymentStatus} />
            <Info label="Estimated Delivery" value={order.estimatedDelivery} />
          </div>
          <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_0.8fr]">
            <div className="rounded-3xl bg-cream p-5">
              ...
              <div className="mt-5 space-y-4">
                {order.items.map((item) => (
                  <div key={item.id} className="flex gap-4 rounded-2xl bg-white p-3">
            ...
            <div className="rounded-3xl bg-ink p-5 text-white">
              ...
              <p className="mt-4 leading-7 text-white/70">{order.address}</p>
              <div className="mt-6 border-t border-white/10 pt-5">
                <p className="text-sm text-white/50">Total Amount</p>
                <p className="mt-1 text-3xl font-bold">{formatCurrency(order.total)}</p>
              </div>
            </div>
          </div>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center print:hidden">
            ...
            <Link to="/auth/login-required" className="inline-flex h-12 items-center justify-center rounded-full bg-linen px-6 text-sm font-bold text-ink transition hover:text-leaf">View Orders</Link>
