...
export default function Login() {
  ...
  const handleSubmit = (event) => {
    ...
    window.setTimeout(() => {
      setLoading(false);
      setError("Demo login is ready for backend authentication.");
    }, 600);
  };
    ...
    <section className="relative grid min-h-[100dvh] place-items-center px-4 py-10">
      ...
      <Container className="grid place-items-center">
        <form className="w-full max-w-md rounded-[2rem] border border-ink/10 bg-white p-6 shadow-soft sm:p-8" onSubmit={handleSubmit} noValidate>
          ...
          <div className="mt-8 grid gap-5"><Input inputRef={emailRef} label="Email" type="email" required autoFocus /><Input label="Password" type="password" required /></div>
          <div className="mt-5 flex items-center justify-between text-sm"><label className="flex items-center gap-2 text-ink/60"><input type="checkbox" /> Remember me</label><a href="#forgot" className="font-semibold text-leaf">Forgot Password?</a></div>
