...
export default function CartSummary({ totals, showCheckout = true }) {
  ...
  const applyCoupon = () => {
    // Backend integration required here: validate coupon against promotions API.
  ...
  return (
    <aside className="rounded-3xl border border-ink/10 bg-white p-6 shadow-sm">
      ...
      {showCheckout && (
        <div className="mt-6 grid gap-2">
          <div className="flex gap-2">
            <Input className="flex-1" placeholder="Coupon code" aria-label="Coupon code" value={coupon} onChange={(event) => setCoupon(event.target.value)} />
            <Button type="button" variant="outline" className="h-[52px] px-4" onClick={applyCoupon}>Apply</Button>
          </div>
      ...
      <div className="mt-6 flex justify-between border-t border-ink/10 pt-5 text-lg font-bold">
        <span>Total</span>
